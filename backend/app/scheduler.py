...
from .models import MonitoringTask,ShoppingItem,StoreListing,PriceSnapshot,PriceAlert,Notification,ShoppingList,User,Store
from .connectors import connector_for
from .services import true_total, get_verified_store_coupons, canonical_store_name
 ...
 try:
  ...
  for task in db.query(MonitoringTask).filter(MonitoringTask.status=="WATCHING").all():
   ...
   for listing in db.query(StoreListing).filter_by(product_id=item.product_id).all():
    ...
    st = db.get(Store, listing.store_id) if listing.store_id else None
    s_canon = canonical_store_name(st.name if st else '')
    coupons = get_verified_store_coupons(s_canon, obs.price, item.name)
    best_coupon = coupons[0] if coupons else None
    coupon_discount = best_coupon.get('discount_amount', 0) if best_coupon else 0
    net_total = max(0.0, total - coupon_discount)

    if item.target_price is not None and (total <= item.target_price or net_total <= item.target_price):
     task.status="TARGET_REACHED"
     if total <= item.target_price:
      msg=f"Target reached for {item.name}: ₹{total:,.0f} on {s_canon or 'store'} (Target: ₹{item.target_price:,.0f})"
     else:
      msg=f"Target reached for {item.name}: ₹{net_total:,.0f} with coupon {best_coupon['code']} on {s_canon or 'store'} (Target: ₹{item.target_price:,.0f})"
     ...
     db.add(PriceAlert(item_id=item.id,alert_type="PRICE_DROP",message=f"Price dropped for {item.name}: ₹{previous:,.0f} → ₹{obs.price:,.0f} on {s_canon or 'store'}"))
